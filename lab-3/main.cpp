...
// COMMENTS TO GRADER: I have used the Model Class
//      from this source in order to load .obj files:
//      https://github.com/WHKnightZ/OpenGL-Load-Model.git
// ============================================================
...
const char TRUMP_OBJ_FILE[] = "./images/trump.obj";
...
const char trumpTexFile[] = "images/trump.png";
...
void MakeReflectionImage(void) {
    ...
    imaginaryViewPos[0] = eyePos[0];
    imaginaryViewPos[1] = eyePos[1];
    imaginaryViewPos[2] = 2 * TABLETOP_Z - eyePos[2];
    
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    glFrustum(TABLETOP_Y1 - imaginaryViewPos[1], TABLETOP_Y2 - imaginaryViewPos[1],
              TABLETOP_X1 - imaginaryViewPos[0], TABLETOP_X2 - imaginaryViewPos[0],
              TABLETOP_Z - imaginaryViewPos[2], TABLETOP_Z - imaginaryViewPos[2] + SCENE_RADIUS);
    
    
    // STEP 3: Sets up the imaginary viewpoint.
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
    gluLookAt(imaginaryViewPos[0], imaginaryViewPos[1], imaginaryViewPos[2],
...
void GLInit(void) {
    ...
    modelTrump.load(TRUMP_OBJ_FILE);
}
...
void SetUpTextureMaps(void) {
    ...
    if (ReadImageFile(woodTexFile, &imageData, &imageWidth, &imageHeight, &numComponents) == 0) {
        exit(1);
    }
    ...
    // This texture object is for the trump texture map.
    glGenTextures( 1, &trumpTexObj );
    glBindTexture( GL_TEXTURE_2D, trumpTexObj );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR );

    if ( ReadImageFile( trumpTexFile, &imageData,
                        &imageWidth, &imageHeight, &numComponents ) == 0 ) exit( 1 );
    if ( numComponents != 3 )
    {
        fprintf(stderr, "Error: Texture image is not in RGB format.\n");
...
void DrawTrump(void) {
    ...
    glTranslated(0.7, -0.6, TABLETOP_Z);
