...
#define RIGHT_ANGLE         90      // Right angle in degrees

#define NSIDES              10      // Number of sides for the torus' individual ring

#define RINGS               8       // Number of rings

#define IR                  1.0     // Inner radius of torus
...
void DrawOneCar(float bodyColor[3]) {
    ...
    glTranslated(0.0, 0.0, -CAR_HEIGHT / 6.0);
    glScaled(CAR_LENGTH, CAR_WIDTH - 4.0, 5.0 * CAR_HEIGHT / 12.0);
    glutSolidCube(1.0);
    ...
    glTranslated(0.0, 0.0, CAR_HEIGHT / 4.0);
    glScaled(CAR_LENGTH / 2.0, CAR_WIDTH - 4.0, 5.0 * CAR_HEIGHT / 12.0);
    glutSolidCube(1.0);
    ...
    glTranslated(CAR_LENGTH / 4.0, -CAR_WIDTH / 2.0 + 1.0, -CAR_HEIGHT / 3.0);
    glRotated(RIGHT_ANGLE, 1.0, 0.0, 0.0);
    glutSolidTorus(IR, CAR_HEIGHT / 6.0, NSIDES, RINGS);
...
void DrawAllCars(void) {
    for (int i = 0; i < NUM_CARS; i++) {
        ...
        // Rotation for a different great circle for each car
        glRotated(car[i].rotAngle, car[i].xzAxis[0], 0.0, car[i].xzAxis[1]);
        
        // Rotation around the globe, given by the initial angular position of the car
        glRotated(car[i].angularPos, 0.0, 1.0, 0.0);
        
        // Translation of the car from origin to the top of the globe
        glTranslated(0.0, 0.0, CAR_TOP_DIST - CAR_HEIGHT / 2.0);
...
void MyDisplay(void) {
    ...
    // Clipping planes separated by `2 * CLIP_PLANE_DISTANCE`
    gluPerspective(VERT_FOV, (double) winWidth / winHeight, eyeDistance - CLIP_PLANE_DIST, eyeDistance + CLIP_PLANE_DIST);

    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();

    // Camera/Eye always `eyeDistance` away from the origin
    // Calculating the projection points onto each axes gives us the coordinates
    // of eye location
    gluLookAt(eyeDistance * cos(eyeLatitude * PI / 180.0) * sin(eyeLongitude * PI / 180.0),
              eyeDistance * sin(eyeLatitude * PI / 180.0),
              eyeDistance * cos(eyeLatitude * PI / 180.0) * cos(eyeLongitude * PI / 180.0),
